...
use crate::astronomy::get_main_sequence_radius_from_mass;
...
#[named]
pub fn get_main_sequence_temperature_from_mass(mass: f64) -> Result<f64, AstronomicalError> {
  trace_enter!();
  trace_var!(mass);
...
pub mod test {
  ...
  #[named]
  #[test]
  pub fn test_get_main_sequence_temperature_from_mass() -> Result<(), AstronomicalError> {
    init();
    trace_enter!();
    // Jolly ol' Sol
    let mut mass = 1.0;
    let mut expected = 5776.0;
    let mut actual = get_main_sequence_temperature_from_mass(mass)?;
    assert_approx_eq!(expected, actual, 1f64);
    // M1V
    mass = 0.40;
    expected = 3407.0;
    actual = get_main_sequence_temperature_from_mass(mass)?;
    assert_approx_eq!(expected, actual, 1f64);
    // K9V
    mass = 0.50;
    expected = 3811.0;
