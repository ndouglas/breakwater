...
#[named]
pub fn get_main_sequence_radius_from_mass(mass: f64) -> Result<f64, AstronomicalError> {
  trace_enter!();
  trace_var!(mass);
...
pub mod test {
  ...
  #[named]
  #[test]
  pub fn test_get_main_sequence_radius_from_mass() -> Result<(), AstronomicalError> {
    init();
    trace_enter!();
    // Jolly ol' Sol
    let mut mass = 1.0;
    let mut expected = 1.0;
    let mut actual = get_main_sequence_radius_from_mass(mass)?;
    assert_approx_eq!(expected, actual);
    // M1V
    mass = 0.40;
    expected = 0.480;
    actual = get_main_sequence_radius_from_mass(mass)?;
